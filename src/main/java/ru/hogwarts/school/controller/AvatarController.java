...
public class AvatarController {
    ...
    public ResponseEntity<String> uploadAvatar(@PathVariable Long id, @RequestParam MultipartFile avatar)
        ...
        if (avatar.getSize() > 1024 * 300) {
            ResponseEntity.badRequest().body("Слишком большой размер");
        }
    ...
    public void downLoadAvatar(@PathVariable Long id, HttpServletResponse response)
        ...
        try (InputStream is = Files.newInputStream(path)) {
            OutputStream os = response.getOutputStream();
            {
                ...
                response.setContentLength((int) avatar
                        .getFileSize(avatar.getFileSize(10000)));
                is.transferTo(os);
            }
        }
    }
    @GetMapping("/page")
    public Collection<Avatar> getPage(@RequestParam int pageNumber,
